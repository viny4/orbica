...
export default async function TopPagesWidget() {
  const data = await fetchAnalytics('/top-pages');

  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-xl h-full">
      <h3 className="text-white/80 font-medium mb-6">Top Pages</h3>
      <div className="space-y-4">
        {data?.length ? data.map((item: any, i: number) => (
          <div key={i} className="flex justify-between items-center">
            <span className="text-sm text-white/60 truncate mr-4">{item.path}</span>
